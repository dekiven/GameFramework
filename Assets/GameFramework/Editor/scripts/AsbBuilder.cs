...
{
    public static string sTempLuaDir = Y3Tools.PathCombine(Application.dataPath, "Lua");
    ...
    public static void BuildAsb(string path, AssetBundleBuild[] builds, BuildAssetBundleOptions opt = BuildAssetBundleOptions.None, BuildTarget target = BuildTarget.StandaloneWindows)
    {
        ...
        {
            path = Y3Tools.PathCombine(path, absFolder);
        }
        ...
        Y3Tools.CheckDirExists(path, true);
    ...
    public static AssetBundleBuild GenBuildByDir(string dir, string relative2, string pattern, bool searchAll = true)
    {
        if (!string.IsNullOrEmpty(dir) && Directory.Exists(dir))
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                {
                    list.Add(Y3Tools.RelativeTo(files[i], Directory.GetParent(Application.dataPath).FullName));
                }
            }
            return GenBundleBuild(Y3Tools.GetAsbName(Y3Tools.RelativeTo(dir, relative2)), list.ToArray());
        }
    ...
    public static List<AssetBundleBuild> GenResBuild(BuilderConfig config)
    {
        ...
        foreach (var d in Directory.GetDirectories(path))
        {
            foreach (var d2 in Directory.GetDirectories(d))
            {
                ...
                {
                    string relativeDir = Y3Tools.RelativeTo(d3, Application.dataPath);
                    Debug.LogWarning(relativeDir);
                    bundles.Add(GenBuildByDir(d3, Y3Tools.GetResPath(), "*.*"));
                }
            }
        }
        return bundles;
    ...
    public static List<AssetBundleBuild> GenLuaBuild(BuilderConfig config)
    {
        ...
        string[] srcDirs = { 
            ...
            , Y3Tools.PathCombine(Application.dataPath, GameConfig.STR_LUA_FOLDER) 
        };
